...
export default function Navbar() {
    ...
    const { isAuthenticated, logout } = useAuth(); // ✅ Auth state & logout
        ...
        <nav className="flex items-center justify-between px-6 sm:px-8 py-4 bg-[#0D1117] sticky top-0 z-50 shadow-[0_0_20px_rgba(30,58,138,0.8)]">
            ...
            <div className="hidden md:flex gap-3">
                {isAuthenticated ? (
                    <button
                        onClick={logout}
                        className="relative px-4 py-2 font-medium rounded-lg text-red-400 border border-red-600 w-[100px] hover:shadow-[0_0_15px_#ef4444] transition duration-500"
                    >
                        Logout
                    </button>
                ) : (
